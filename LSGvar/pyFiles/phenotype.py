...
def compute_variant_similarity(v1, v2, max_distance=500, small_distance=10):
    ...
    if v1.var_type == 'SNV':
        ...
        if svtype == 'INS':
            ...
            min_len = min(v1.end-v1.pos, v2.end-v2.pos)
        ...
        for cluster in clusters:
            ...
            if len(cluster) == 1:
                ...
                if v1.haplotype == v2.haplotype:
                    ...
                    variant = v1 if qual1 >= qual2 else v2
                    variant.genotype = '1|1' if variant.haplotype == 1 else '1|1'
                    variant.id = merged_id
                    variant.info = base_info  
                    variant.pos = min_pos
                    variant.end = max_end
                ...
                if hap1_vars:
                    best_hap1 = max(hap1_vars, key=lambda v: float(v.qual) if v.qual != '.' else 0)
                else:
                    best_hap1 = None
                
                if hap2_vars:
                    best_hap2 = max(hap2_vars, key=lambda v: float(v.qual) if v.qual != '.' else 0)
                else:
                    best_hap2 = None

                if best_hap1 and best_hap2:
                    qual1 = float(best_hap1.qual) if best_hap1.qual != '.' else 0
                    qual2 = float(best_hap2.qual) if best_hap2.qual != '.' else 0
                    variant = best_hap1 if qual1 >= qual2 else best_hap2
                    ...
                    best_var.genotype = '1|1' if best_hap1 and best_hap2 else ('1|0' if best_hap1 else '0|1')
                    best_var.id = merged_id
                    best_var.info = base_info  
                    best_var.pos = min_pos
                    best_var.end = max_end
                    if best_var.var_type == 'SV' and 'END' in base_info:
                        base_info['END'] = str(max_end)
                    merged_variants.append(best_var)
...
def write_vcf_header(output_handle, args, sample_name="SAMPLE"):
    ...
    output_handle.write("##source=LSGvar\n")
