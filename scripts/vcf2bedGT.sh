...
paste <(less -S $file |grep 'INS' |awk 'OFS="\t"{print $1,$2,$3}')  <(less -S $file |grep 'INS' |awk -F'[-\t]'  'OFS="\t"{print $2+1,"INS",$6,"."}' |less -S) <(less -S $file |grep 'INS'|cut -f10)   <(less -S $file |grep 'INS'|awk -F'TIG_REGION=' '{print $2}' |less -S |awk -F',' '{print $1}') >INS.bed
paste <(less -S $file |grep 'DEL' |awk  'OFS="\t"{print $1，$2，$3}') <(less -S $file |grep 'DEL' |awk -F'[-\t]'  'OFS="\t"{print $2+$6，"DEL",$6，"."}' |less -S) <(less -S $file |grep 'DEL'|cut -f10)   <(less -S $file|grep 'DEL'  |awk -F'TIG_REGION=' '{print $2}' |less -S |awk -F',' '{print $1}') >DEL.bed
paste <(less -S $file |grep 'INV' |awk  'OFS="\t"{print $1，$2，$3}') <(less -S $file |grep 'INV' |awk -F'[-\t]' 'OFS="\t"{print $2+$6，"INV",$6，"."}' |less -S) <(less -S $file |grep 'INV'|cut -f10)   <(less -S $file|grep 'INV'  |awk -F'TIG_REGION=' '{print $2}' |less -S |awk -F',' '{print $1}') >INV.bed
